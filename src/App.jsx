...
function App() {
  ...
  const [inputActiveStatus, setInputActiveStatus] = useState("");
  ...
  useEffect(
    function () {
      async function fetchWeather() {
        const url = `https://weatherapi-com.p.rapidapi.com/current.json?q=${"Ethipia"}`;
        const options = {
          method: "GET",
          headers: {
            "X-RapidAPI-Key":
  ...
  useEffect(
    function () {
      function callback(e) {
        if (inputActiveStatus && e.code === "Enter") setEnter(true);
      }
    ...
    <div className="mx-[3vw]">
      ...
      <main className="mt-8">
        ...
        <div className=" grid grid-cols-4 grid-rows-3 gap-3 h-[400px] ">
          <div className="row-span-3 pt-11 bg-slate-300 pl-[40px]  border rounded-xl border-slate-700 mx-11">
            <svg
            ...
            <p className="text-4xl mt-8 font-medium">28.6&deg;C</p>
          ...
          <WeatherItem property={"Wind"} amount={""} />
          <WeatherItem property={"Pressure"} amount={"1008pa"} />
          <WeatherItem property={"Humidity"} amount={"48%"} />
          ...
          <div className="row-start-1 col-start-4 row-span-3 bg-slate-300 flex items-center justify-center rounded-xl mx-2">
            8
          </div>
        </div>
      </main>
...
function WeatherItem({ property, amount }) {
  return (
    <div className=" bg-slate-300 flex items-center justify-center rounded-xl border-slate-700 flex-col m-2">
      <p>{property}</p>
      <p>{amount}</p>
    </div>
  );
